...
class LISTEN():
    ...
    def startListen(rec):
        ...
        while True:
            ...
            #-----------------------Check if no signal------------------------
            if rec.dtmf_to_hexa(highestfreqs).size>0 and rec.startReading==True:
                rec.noSignal+=1
                if rec.noSignal>5:
                    break
            else:
            ...
            #-----------------------Check if finished--------------------------
            if np.array_equal(rec.outputList,[0xC,0xC]) and rec.syncCounter>5:
            ...
            #-----------------------Check if succesful-------------------------
            if np.array_equal(rec.outputList, [0xa,0xb])and not(rec.startReading):
                print("Synchronized")
                rec.outputList=np.array([],dtype=int)
                rec.syncCounter+=1
            ...
            #-----------------------Check if failure--------------------------
            if not(np.array_equal(rec.outputList,[0xa,0xb])) and len(rec.outputList)==2 and not(rec.startReading):
