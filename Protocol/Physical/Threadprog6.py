...
class LISTEN():
    def __init__(rec, baud, sync=20, fade=0.006667, amplitude=15000, senderFs=44100, pack=[0, 1, 1, 7, 0, 8, 6, 10, 2, 0, 1, 0, 1, 2, 8, 0, 8, 10, 7, 9, 3, 0, 1, 0, 1, 3, 4, 13, 6, 5, 7, 3, 7, 3, 6, 1, 6, 7, 0, 8, 7, 0, 1, 0, 1, 4, 6, 5, 15, 5, 4, 0, 1]):
        ...
        rec.tooLow=0
        rec.threshhold=5000
        rec.tones=np.array([], dtype=int)
        
        #for log
        rec.logFreqs=np.array([[99,99,99,99,99]])    
        rec.getLog=True
    ...
    def startListen(rec):
        ...
        while(True):
            while(not(rec.starting)):
                data = rec.stream.read(1024, exception_on_overflow=True)
                data_int = np.frombuffer(data,dtype="h")
                print(np.amax(data_int))
                if np.amax(data_int)>5000:
                    rec.starting=True
                    rec.tooHigh+=1
                else:
                    rec.tooHigh=0
            ...
            first12=np.where(rec.tones==12)
            print(first12[0])
            rec.tones=np.delete(rec.tones,np.arange(first12[0][0]))
            #delete the 12 for sync
            rec.tones=np.delete(rec.tones,[0])
